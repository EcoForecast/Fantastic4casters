...
rm(qa_le)
qa_vswc = joint_out$data$VSWC_obs[!is.na(joint_out$data$VSWC_obs)]
...
bin = 86400 / 48          ## seconds per day / timestep = number of timesteps per day
#out.daily = array(0.0, c(ne, ceiling(nt/bin), 3))  ## [day, ensemble, variables]
out.daily = array(0.0, c(ne, ceiling(nt/48),  3))
for(i in 1:3){                 ## loop over each variable
  #print(i)
  out.daily[,,i] <- apply(output[,,i],1, ctapply, rep(1:365,each=bin)[1:nt], mean)
}

## Basic time-series visualizations
varnames <- c("NEE", "LE", "VSWC")
units <- c("units", "units", "units")

for(i in 1:3){  ## loop over variables
  ci = apply(out.daily[, , i], 2, quantile, c(0.025, 0.5, 0.975))   ## calculate CI over ensemble members
