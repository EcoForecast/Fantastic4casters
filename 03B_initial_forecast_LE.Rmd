...
sw.mean <- apply(sw_driver_gf,2,mean) ## driver
lw.mean <- apply(lw_driver_gf,2,mean)
## parameters
...
## function test
n = 1 ## Deterministic prediction
Q = 0 ## no process error
N <- matrix(NA,1,timeN_predict)
IC_tmp = mean(IC[,"x[17520]"])
Nprev <- IC_tmp
for(t in 1:timeN_predict){
  mu = Nprev + param.mean["betaIntercept"] + param.mean["betaX"]*Nprev + param.mean["betainlw"]*sw.mean[t] + param.mean["betainsw"]*lw.mean[t]
  N[t] <- rlnorm(n,mu,Q)
  Nprev <- N[t]
}
