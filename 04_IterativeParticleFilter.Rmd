...
IC_LE = sample(joint_out$predict[[2]][,17520], ne)
IC_NEE = sample(joint_out$predict[[2]][,35040], ne)
IC_VSWC = sample(joint_out$predict[[2]][,52560], ne)
...
for(i in 1:ne){
  ...
  #for now just using standard dev 1 from lack of standard dev data
  
  NEElike[i, is.na(NEElike[i, ])] = 0       ## missing data as weight 1; log(1)=0
  print(length(exp(cumsum(NEEcalc[i, ]))))
